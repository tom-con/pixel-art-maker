document.addEventListener("DOMContentLoaded", function() {
  ...
  var allDivs;
  var lastDiv = document.getElementsByClassName('last')[0];
  var save = [];
  var load = [];
  var clicker = {
    'current': ""
  ...
  function makeDivs(columns, rows, width, height) {
    ...
    allDivs = document.getElementsByClassName('exists');
    lastDiv = allDivs[allDivs.length-1];
    lastDiv.style.border = "1px red solid";
    lastDiv.classList.add("last");
    lastDiv.addEventListener("mousedown", downCoords);
    return itemsArr;
  }
  ...
  var applyColor = function() {
    if (event.target.className === 'exists') {
...
var upCoords = function() {
  var ux = event.clientX;
  var uy = event.clientY;
  console.log("upX: ", ux);
  console.log("upY: ", uy);
  var diffX = ux - this.x;
  var diffY = uy - this.y;
  var addX = (diffX-diffX%board.wid)/board.wid;
  var addY = (diffY-diffY%board.hgt)/board.hgt;
  var rowz = this.childNodes.length;
  var colz = this.childNodes[0].childNodes.length;
  var newX = addY + rowz;
  var newY = addX + colz;

  save = makeDivs(newY, newX, board.wid-2, board.hgt-2);
  board.removeEventListener("mouseup", upCoords);
};

var downCoords = function(){
  board.x = event.clientX;
  board.y = event.clientY;
  board.wid = parseInt(event.target.style.width.substring(0,event.target.style.width.length-2)) + 2;
  board.hgt = parseInt(event.target.style.height.substring(0,event.target.style.height.length-2)) + 2;
  board.addEventListener("mouseup", upCoords);
};
